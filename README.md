...
- `index.html` is the participant experience. One shared test link allows approved testers to create a non-empty username and password without character, case, or length rules. Login sessions are stored only on that device, and each account reads and writes only its own records.
